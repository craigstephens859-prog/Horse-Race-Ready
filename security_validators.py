...
# =====================================================
# NOTE: Patterns must not match legitimate BRISNET formatting
# BRISNET uses "open(" in workout quality markers (e.g., "open(Dmr")
CODE_INJECTION_PATTERNS = [
    re.compile(r'__import__\s*\(', re.IGNORECASE),  # More specific
    re.compile(r'eval\s*\(', re.IGNORECASE),
    re.compile(r'exec\s*\(', re.IGNORECASE),
    re.compile(r'compile\s*\(', re.IGNORECASE),
    # Removed 'open\s*\(' - conflicts with BRISNET workout notation
    re.compile(r'os\.system\s*\(', re.IGNORECASE),  # More specific
    re.compile(r'subprocess\.', re.IGNORECASE),
    re.compile(r'__builtins__', re.IGNORECASE),  # Added
    re.compile(r'import\s+os\b', re.IGNORECASE),  # Added
]
...
def sanitize_pp_text(text: str) -> str:
    ...
    # 2. Remove ONLY dangerous control characters
    # Keep printable characters, newlines (\n), tabs (\t), and racing symbols
    # Remove: NULL, backspace, escape sequences, etc. (0x00-0x08, 0x0E-0x1F)
    # Keep: Tab (0x09), LF (0x0A), CR (0x0D), and all printable (>= 0x20)
    # Keep: Extended ASCII and Unicode for racing symbols (ñ, ×, √, etc.)
    text = re.sub(r'[\x00-\x08\x0e-\x1f]', '', text)  # Removed 0x0b-0x0c (keep LF/CR), removed 0x7f-0x9f range
    ...
    for pattern in CODE_INJECTION_PATTERNS:
        if pattern.search(text):
            ...
            raise ValueError("Invalid code patterns detected in PP text")
