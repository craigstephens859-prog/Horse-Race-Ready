...
def parse_brisnet_header(pp_text: str) -> dict[str, Any]:
    ...
    try:
        ...
        if is_pipe_format:
            ...
            age_pattern = (
                r"^((?:\d+)?(?:yo|&up)?(?:\s+(?:Fillies|Colts|Mares|F&M|C&G))?)"
            )
            ...
            if result["distance"]:
                result["distance_furlongs"] = _convert_distance_to_furlongs(
                    result["distance"]
                )
        ...
        for i, part in enumerate(parts):
            ...
            if not result["track_name"] and i > 0:
                if not re.search(
                    r"\b(race|furlong|mile|yard|stakes|allowance|claiming|maiden)\b",
                    part_lower,
                ):
            ...
            sex_match = re.search(
                r"(fillies?|mares?|colts?|geldings?|f&m|c&g)", part_lower
            )
...
def parse_race_conditions(
    race_conditions: str, purse_amount: int = 0
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if nw_match:
            result["additional_conditions"].append("Non-Winners Condition")
...
def get_hierarchy_level(
    class_type: str, grade_level: int | None = None, purse_amount: int = 0
) -> dict[str, Any]:
    """
    ...
    - Grade 1: Base level 7 + 3 boost = 10
    - Grade 2: Base level 7 + 2 boost = 9
    - Grade 3: Base level 7 + 1 boost = 8
    ...
    try:
        ...
        # Apply grade boost
        # FIX (Feb 13, 2026): Restore G1→+3 to separate G1 from G2.
        # Pegasus WC analysis showed form/speed matter in elite races, but
        # collapsing G1=G2 (both +2) eliminated class differentiation entirely.
        # A $3M G1 and $250K G2 must not score identically.
        if grade_level == 1:
            result["grade_boost"] = 3  # G1: highest prestige
        elif grade_level == 2:
            result["grade_boost"] = 2  # G2: high prestige
        elif grade_level == 3:
            result["grade_boost"] = 1  # G3: solid prestige
        else:
        ...
        result["adjusted_level"] = round(
            result["final_level"] + result["purse_adjustment"], 2
        )
...
[Additional PP data would follow...]
    ...
    print(
        f"Distance: {result_1['summary']['distance']} ({result_1['summary']['distance_furlongs']} furlongs)"
    )
    print(
        f"Hierarchy Level: {result_1['hierarchy']['base_level']} + {result_1['hierarchy']['grade_boost']} boost = {result_1['hierarchy']['final_level']}"
    )
...
[Additional PP data would follow...]
    ...
    print(
        f"Distance: {result_3['summary']['distance']} ({result_3['summary']['distance_furlongs']} furlongs)"
    )
...
[Additional PP data would follow...]
    ...
    print(
        f"Distance: {result_4['summary']['distance']} ({result_4['summary']['distance_furlongs']} furlongs)"
    )
    print(
        f"Hierarchy Level: {result_4['hierarchy']['base_level']} + {result_4['hierarchy']['grade_boost']} boost = {result_4['hierarchy']['final_level']}"
    )
...
[Additional PP data would follow...]
    ...
    print(
        f"Distance: {result_5['summary']['distance']} ({result_5['summary']['distance_furlongs']} furlongs)"
    )
