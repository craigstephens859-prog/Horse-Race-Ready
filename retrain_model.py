...
class RaceDataset(Dataset):
    ...
    def __init__(self, features_df: pd.DataFrame, labels_df: pd.DataFrame):
        ...
        Args:
            ...
            labels_df: DataFrame with race_id, horse_id, actual_finish_position
        """
        self.races = []
        ...
        for race_id, race_data in race_groups:
            ...
            race_combined = race_data.merge(
                race_labels[['horse_id', 'actual_finish_position']],
                on='horse_id',
                how='inner'
            )
            ...
            feature_cols = [c for c in race_combined.columns 
                           if c not in ['race_id', 'horse_id', 'actual_finish', 'actual_finish_position']]
...
def plackett_luce_loss(scores: torch.Tensor, true_rankings: torch.Tensor) -> torch.Tensor:
    """
...
def retrain_model(
    ...
    if training_data is None:
        logger.error(f"❌ Insufficient data. Need at least {min_races} completed races.")
        return {'error': 'Insufficient data'}
    
    features_df, labels_df = training_data
    
    logger.info(f"✅ Loaded {len(features_df)} horses from {len(features_df['race_id'].unique())} races")
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_races)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_races)
    
    logger.info(f"📊 Train: {n_train} races | Val: {n_val} races")
    
    # 3. Initialize model
    n_features = len([c for c in features_df.columns 
                     if c not in ['race_id', 'horse_id', 'actual_finish']])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min-races', type=int, default=50, help='Minimum races required')
