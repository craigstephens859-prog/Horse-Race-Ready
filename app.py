...
def _distance_bucket_from_text(distance_txt: str) -> str:
    ...
    if "furlong" in d:
        ...
        if m := re.search(r"(\d+(?:\.\d+)?)", s):
            ...
            if val <= 6.0:
                return "≤6f"
            return "6.5–7f" if val < 8.0 else "8f+"
    # Miles
    if "mile" in d and "70" in d and "yard" in d:
        return "8f+"
        fracs = {
            "1/16": 1 / 16,
            "1/8": 1 / 8,
...
def parse_track_name_from_pp(pp_text: str) -> str:
    ...
    for canon, toks in TRACK_ALIASES.items():
        for t in toks:
            ...
            if t_words and all(
                re.search(rf"\b{re.escape(w)}\b", head) for w in t_words
            ):
                return canon
    return ""
...
def extract_morning_line_by_horse(pp_text: str) -> Dict[str, str]:
    ...
    for name, block in blocks.items():
        if name in ml:
            continue
        if m_start := re.search(rf"(?mi)^\s*{_ODDS_TOKEN}", block or ""):
            ml[name.strip()] = m_start.group(1).replace(" ", "")
            continue
        if m_labeled := re.search(
...
def parse_pedigree_snips(block: str) -> dict:
    ...
    if d := re.search(r"(?mi)^\s*Dam:\s*DPI\s*(\d+(?:\.\d+)?)\s+(\d+)%", block or ""):
...
def parse_trainer_jockey_stats(block: str) -> dict:
    """
    Parses trainer and jockey win percentages from BRISNET PP block.
    Format: "2025 177 14% 37% -0.01" where 14% is win%, 37% is ITM%
    """
    out = {"trainer_win_pct": None, "jockey_win_pct": None}
    
    lines = (block or "").split('\n')
    
    for i, line in enumerate(lines):
        # Look for jockey stats lines (usually have JKYw/ prefix or appear after jockey name)
        # Format: "2025 177 14% 37% -0.01" or "JKYw/EP types 346 17% 44% -0.68"
        if i < len(lines) - 1:
            next_line = lines[i + 1]
            
            # Check if current line has jockey name with win%
            if 'JOCKEY' in line.upper() or any(x in line for x in ['JKYw/', 'Jockey']):
                # Next few lines might have stats
                if m := re.search(r'^\s*\d{4}\s+\d+\s+(\d{1,2})%\s+\d{1,2}%', next_line):
                    try:
                        out["jockey_win_pct"] = float(m.group(1))
                    except:
                        pass
            
            # Look for trainer stats (Trnr: line followed by stats)
            if 'Trnr:' in line or 'TRAINER' in line.upper():
                if m := re.search(r'^\s*\d{4}\s+\d+\s+(\d{1,2})%\s+\d{1,2}%', next_line):
                    try:
                        out["trainer_win_pct"] = float(m.group(1))
                    except:
                        pass
...
def parse_workout_info(block: str) -> str:
    """
    Parses recent workout information from BRISNET PP block.
    Format: "24Oct Sar tr.t 4f ft :48« B" or "×19Oct SA 5f ft :59© H"
    Returns a string with workout dates/times for counting.
    """
    workouts = []
    
    # Pattern for workout lines: optional ×, date (DDMon), track, distance, surface, time
    # Example: "24Oct SA 5f ft 1:00¨ H" or "×19Oct SA 5f ft :59© H"
    pattern = r'(?:×)?(\d{2}[A-Z][a-z]{2})\s+\w+\s+(?:tr\.t\s+)?(\d+f?)\s+\w+\s+([\d:\.]+)'
    
    for m in re.finditer(pattern, block or ""):
        date = m.group(1)
        distance = m.group(2)
        time = m.group(3)
        workouts.append(f"{date} {distance} {time}")
    
    return " ".join(workouts[:6])  # Return up to 6 most recent


def parse_days_since_last_race(block: str) -> Optional[int]:
    """
    Calculates days since last race by finding the most recent race date in DATE TRK lines.
    Format: "04Oct25Baq 1m ft :22© :44«1:09¨ 1:35«"
    Returns number of days from today, or None if can't determine.
    """
    from datetime import datetime
    
    # Look for race dates in format "DDMmmYY" at start of race lines
    # Example: "04Oct25Baq" or "30Aug25Sar¨¨"
    date_pattern = r'^(\d{2})([A-Z][a-z]{2})(\d{2})[A-Z]'
    
    matches = []
    for line in (block or "").split('\n'):
        if m := re.match(date_pattern, line.strip()):
            matches.append(m)
    
    if not matches:
        return None
    
    # Parse the first (most recent) date
    try:
        first_match = matches[0]
        day = int(first_match.group(1))
        month_str = first_match.group(2)
        year = int("20" + first_match.group(3))
        ...
        last_race_date = datetime(year, month, day)
        today = datetime.now()
        days_diff = (today - last_race_date).days
        
        # Sanity check: return None if negative or unreasonably large
        if 0 <= days_diff <= 365:
            return days_diff
    except:
        pass
...
def compute_ppi(df_styles: pd.DataFrame) -> dict:
    ...
    ppi_val = (
        (counts["E"] + counts["E/P"] - counts["P"] - counts["S"])
        * MODEL_CONFIG["ppi_multiplier"]
        / total
    )
...
def apply_enhancements_and_figs(
    ...
    pp_text: str,
    processed_weights: Dict[str, float],
    chaos_index: float,
    track_name: str,
    surface_type: str,
    distance_txt: str,
...
def calculate_exotics_biased(
    ...
    for r in ex_rows:
        r["Fair Odds"] = (1.0 / r["Prob"]) - 1 if r["Prob"] > 1e-9 else float("inf")
...
if parse_clicked:
    if not (text_now := (st.session_state.get("pp_text_input") or "").strip()):
...
surface_type = st.selectbox(
    ...
    ["Dirt", "Turf", "Synthetic"],
    index=(
        ["Dirt", "Turf", "Synthetic"].index(default_surface)
...
for cond in conditions:
    if re.search(rf"(?i)\b{cond}\b", first_line):
        cond_found = cond
        break
default_condition = cond_found or st.session_state["condition_txt"]
...
def _auto_distance_label(s: str) -> str:
    if m := re.search(r"(?i)\b(\d+(?:\s*1/2|½)?\s*furlongs?)\b", s):
...
def detect_purse_amount(pp_text: str) -> Optional[int]:
    s = pp_text or ""
    if m := re.search(r"(?mi)\bPurse\b[^$\n\r]*\$\s*([\d,]+)", s):
        try:
            return int(m.group(1).replace(",", ""))
        except:
            pass
    if m := re.search(r"(?mi)\b(?:Added|Value)\b[^$\n\r]*\$\s*([\d,]+)", s):
        try:
            return int(m.group(1).replace(",", ""))
        except:
            pass
...
trainer_jockey_per_horse: Dict[str, dict] = {}
workouts_per_horse: Dict[str, str] = {}
days_since_per_horse: Dict[str, Optional[int]] = {}
...
for _post, name, block in split_into_horse_chunks(pp_text):
    if name in df_editor["Horse"].values:
        ...
        trainer_jockey_per_horse[name] = parse_trainer_jockey_stats(block)
        workouts_per_horse[name] = parse_workout_info(block)
        days_since_per_horse[name] = parse_days_since_last_race(block)
...
df_final_field["StyleStrength"] = df_final_field.apply(
    lambda row: calculate_style_strength(
        row["OverrideStyle"] or row["DetectedStyle"], row["Quirin"]
    ),
    axis=1,
)
df_final_field["Style"] = df_final_field.apply(
    lambda r: _normalize_style(r["OverrideStyle"] or r["DetectedStyle"]), axis=1
)
if "#" not in df_final_field.columns:
    df_final_field["#"] = df_final_field["Post"].astype(str)

# ===================== ADD MISSING ANGLE DATA FIELDS =====================

# 1. LastFig - Most recent speed figure
def get_last_fig(horse_name):
    figs = figs_per_horse.get(horse_name, [])
    return figs[0] if figs else None

df_final_field["LastFig"] = df_final_field["Horse"].apply(get_last_fig)

# 2. SireROI - Calculate from sire first-time starter percentage
def get_sire_roi(horse_name):
    ped = pedigree_per_horse.get(horse_name, {})
    sire_1st = ped.get("sire_1st")
    if sire_1st is not None and not pd.isna(sire_1st):
        # Convert 1st% to a rough ROI proxy (15%+ is good, scale it)
        return max(0, (sire_1st - 10) * 2)  # Rough conversion
    return None

df_final_field["SireROI"] = df_final_field["Horse"].apply(get_sire_roi)

# 3. TrainerWin% and JockeyWin%
def get_trainer_win_pct(horse_name):
    stats = trainer_jockey_per_horse.get(horse_name, {})
    return stats.get("trainer_win_pct")

def get_jockey_win_pct(horse_name):
    stats = trainer_jockey_per_horse.get(horse_name, {})
    return stats.get("jockey_win_pct")

df_final_field["TrainerWin%"] = df_final_field["Horse"].apply(get_trainer_win_pct)
df_final_field["JockeyWin%"] = df_final_field["Horse"].apply(get_jockey_win_pct)

# 4. Workouts - Recent workout string
def get_workouts(horse_name):
    return workouts_per_horse.get(horse_name, "")

df_final_field["Workouts"] = df_final_field["Horse"].apply(get_workouts)

# 5. DaysSinceLast - Days since last race
def get_days_since(horse_name):
    return days_since_per_horse.get(horse_name)

df_final_field["DaysSinceLast"] = df_final_field["Horse"].apply(get_days_since)

# ===================== END ANGLE DATA FIELDS =====================
...
def _infer_horse_surface_pref(
    name: str, ped: dict, ang_df: Optional[pd.DataFrame], race_surface: str
) -> str:
    cats = (
...
def _angles_pedigree_tweak(
    ...
    if race_bucket == "≤6f":
        for v in [ped.get("sire_1st"), ped.get("damsire_1st")]:
            if pd.notna(v) and float(v) >= MODEL_CONFIG["ped_first_pct_threshold"]:
    ...
    # 3) Condition nuance
    if (
        race_cond in {"muddy", "sloppy", "heavy"}
        and awd_mean == awd_mean
        and race_bucket != "≤6f"
        and awd_mean >= 7.5
    ):
        ...
        rows.append(
            {
                ...
                "Arace": round(arace, 2),
                "R": round(R, 2),
                # Pass through angle data fields if they exist
                "LastFig": row.get("LastFig"),
                "SireROI": row.get("SireROI"),
                "TrainerWin%": row.get("TrainerWin%"),
                "JockeyWin%": row.get("JockeyWin%"),
                "Workouts": row.get("Workouts"),
                "DaysSinceLast": row.get("DaysSinceLast"),
            }
        )
...
def fair_probs_from_ratings(ratings_df: pd.DataFrame) -> Dict[str, float]:
    if (
        ratings_df is None
        or ratings_df.empty
        or "R" not in ratings_df.columns
        or "Horse" not in ratings_df.columns
    ):
        return {}
    ...
    if pd.isna(median_r):
        median_r = 0  # Handle case where all are NaN
...
def get_weight_preset(surface, distance):
    return {"class_form": 1.0, "trs_jky": 1.0}


def apply_strategy_profile_to_weights(w, profile):
    return w


def adjust_by_race_type(w, rt):
    return w


def apply_purse_scaling(w, purse):
    return w or {}
...
if scenarios:
    ...
    if primary_key in all_scenario_ratings:
        ...
        st.info(
            f"**Primary Scenario:** S: `{primary_key[0]}` • P: `{primary_key[1]}` • Profile: `{strategy_profile}`  • PPI: {ppi_val:+.2f}"
        )
    else:
...
# ===================== B2. Eight-Angle Enhancement =====================
if not primary_df.empty:
    try:
        # Log available columns for debugging
        st.caption(f"🔍 DataFrame columns available: {', '.join(primary_df.columns.tolist())}")
        
        # Call compute_eight_angles with debug=True to get mapping info
        angles_result = compute_eight_angles(primary_df, column_map=None, debug=True)
        
        # Unpack result
        if isinstance(angles_result, tuple):
            angles_df, mapping_df = angles_result
        else:
            # Fallback if debug returns single DataFrame
            angles_df = angles_result
            mapping_df = pd.DataFrame()
        
        # Optionally blend angles into ratings (adjust weight as needed)
        primary_df = apply_angles_to_ratings(primary_df, angles_df, weight=0.25)

        # Display angle breakdown
        with st.expander("📊 View 8-Angle Breakdown", expanded=False):
            st.caption("Normalized angle scores (0-1 scale) for each horse:")
            
            # Show column mapping table
            if not mapping_df.empty:
                st.subheader("🗺️ Angle Column Mapping")
                st.dataframe(
                    mapping_df,
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "Canonical": st.column_config.TextColumn("Expected Field", width="medium"),
                        "Resolved": st.column_config.TextColumn("Actual Column", width="medium"),
            ...
            # Show angle scores
            st.subheader("📈 Angle Scores")
            st.dataframe(
                angles_df.round(3),
                use_container_width=True,
                column_config={
                    "Angle_SourceCount": st.column_config.NumberColumn(
                        "Source Count", format="%d", help="Number of non-missing features used"
                    ),
                    "Angles_Total": st.column_config.NumberColumn(
                        "Total Score", format="%.3f"
                    )
                },
            )
    except Exception as e:
        st.warning(f"⚠️ Angle calculation skipped: {e}")
        import traceback
...
for _, r in df_final_field.iterrows():
    ...
    if dec := str_to_decimal_odds(odds_str):
...
def build_betting_strategy(
    ...
    is_underlay = (
        not is_overlay
        and (primary_probs.get(top_rated_horse, 0) > (1 / top_ml_dec))
        and top_ml_dec < 4
...
if st.button("Analyze This Race", type="primary", key="analyze_button"):
    with st.spinner("Handicapping Race..."):
        try:
            ...
            if primary_df.empty or not all(
                col in primary_df.columns
                for col in ["Horse", "R", "Fair %", "Fair Odds"]
            ):
            ...
            name_to_ml = pd.Series(
                df_final_field["ML"].values, index=df_final_field["Horse"]
            ).to_dict()
