...
MODEL_CONFIG = {
    ...
    "speed_fig_weight": 0.15,  # OPTIMIZED: Was 0.05. 0.15 = 10 fig points = 1.5 bonus (Beyer/Benter research).
...
def calculate_form_cycle_rating(
    ...
    return float(np.clip(form_rating, -2.0, 3.0))  # OPTIMIZED: floor -2.0 (was -3.0)
...
def calculate_hot_trainer_bonus(
    ...
    # OPTIMIZED: 0% trainer penalty capped at -1.2 (was -2.5)
    ...
    **Troubleshooting:**
    ...
    try:
        ...
        tab_overview, tab_results, tab_calibration, tab_retrain = st.tabs(
            ["📊 Dashboard", "🏁 Submit Actual Top 4", "🤖 Auto-Calibration Monitor", "🚀 Retrain Model"]
        )
                    ...
                    if not horses:
                        ...
                        if submitted and finish_input and finish_input.strip():
                            ...
                            try:
                                ...
                                if finish_order and len(finish_order) != len(set(finish_order)):
                                ...
                                if finish_order and len(finish_order) >= 4:
                                    ...
                                    for i, pos in enumerate(finish_order[:4]):
                                        horse_name = horse_names_dict.get(pos, f'Horse #{pos}')
                                        medals = ["🥇", "🥈", "🥉", "4th"]
